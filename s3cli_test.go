...
// presignV2Escaped gen a presigned URL with escaped key(Object name).
func presignV2Escaped(method, server, bucket, key, ak, sk, contentType string, exp int64) (string, error) {
...
func TestMyTest(t *testing.T) {
	...
	u1, err := presignV2Raw(http.MethodGet, s, b, k, ak, sk, "", 999999999)
	if err != nil {
		t.Errorf("error1: %s", err)
	}
	t.Error(u1)

	u2, err := presignV2Escaped(http.MethodGet, s, b, k, ak, sk, "", 999999999)
	if err != nil {
